...
export default class extends ExtensionPreferences {
    ...
    #gradientPreview?: InstanceType<typeof GradientPreviewWidget>;
    ...
    async fillPreferencesWindow(window: Adw.PreferencesWindow): Promise<void> {
        ...
        this.#gradientPreview.connect('stop-selected', (_widget: any, _index: number) => {
            GradientUtils.saveGradientStops(this.#settings!, this.#gradientStops);
            this.#rebuildColorStopsList();
        });
        
        this.#gradientPreview.connect('stop-position-changed', (_widget: any, index: number, position: number) => {
        ...
        this.#gradientPreview.connect('stop-added', (_widget: any, position: number) => {
        ...
        const updateAngleVisibility = (): void => {
        ...
        const updateSeedVisibility = (): void => {
            const currentType = settings.get_string('gradient-type');
        ...
        const updateNoiseVisibility = (): void => {
            const currentType = settings.get_string('gradient-type');
    ...
    #rebuildColorStopsList(): void {
        if (!this.#colorStopsListBox) return;
    ...
    #createColorStopRow(stop: GradientStop, index: number): Adw.ActionRow {
      ...
      #addColorStopAtPosition(position: number): void {
